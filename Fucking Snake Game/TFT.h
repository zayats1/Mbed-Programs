...
public:
   ...
   */
  TFT(proto_t displayproto, PortName port, PinName CS, PinName reset,
      PinName DC, PinName WR, PinName RD, const int lcdsize_x,
      const int lcdsize_y, const char *name);

  /** Create a TFT Parallel Bus interface
   * @param name The name used by the parent class to access the interface
   */
  TFT(proto_t displayproto, PinName *buspins, PinName CS, PinName reset,
      PinName DC, PinName WR, PinName RD, const int lcdsize_x,
      const int lcdsize_y, const char *name);
   ...
   * @param name The name used by the parent class to access the interface
   */
  TFT(proto_t displayproto, int Hz, PinName mosi, PinName miso, PinName sclk,
      PinName CS, PinName reset, PinName DC, const int lcdsize_x,
      const int lcdsize_y, const char *name);

  /////// functions that come for free, but can be
  /// overwritten///////////////////////////////////////////////////
  ///////
  ///----------------------------------------------------///////////////////////////////////////////////////
   ...
   * used to speedup functions that plots single pixels, like circle, oblique
   * lines or just sparse pixels
   * @param enable true/false
   * @note most but not all controllers support this, even if datasheet tells
   * they should
   */
   ...
   * TFTs are mainly native protrait view, so horizontal scroll if rotated in
   * landscape view
   *
   * @param startY boundary offset from top (or left if rotated), 0 for
   * fullscreen scroll
   * @param areasize size of the scroll area, 480 for fullscreen scroll of a
   * 320x480 display
   */
   ...
   *
   * @param lines number of lines to scroll, 1= scrollup 1, areasize-1=
   * scrolldown 1
   */
  ...
  /** (Don't work) gets display X size in pixels (native, orientation independent)
   * @returns X size in pixels
   */
  virtual int getSizeX();

  /**  (Don't work) gets display Y size in pixels (native, orientation independent)
...
protected:
 ...
 int screensize_Y;
   ...
   * @note if protocol is SPI16, it will insert NOP cmd before, so if cmd is a
   * 2byte cmd, the second cmd will be broken. Use wr_cmd16 for 2bytes cmds
   */
   ...
   * @returns data as uint
   * @note EXTC regs (0xB0 to 0xFF) are read/write registers but needs special
   * cmd to be read in SPI mode
   */
   ...
   * @note support ILI9341,94xx, MIPI standard. May be be overridden in Init
   * class for other specific IC
   */
